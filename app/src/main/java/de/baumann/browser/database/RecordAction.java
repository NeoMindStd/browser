...
public class RecordAction {
	...
	public boolean addStartSite(Record record) {
		...
		values.put(RecordUnit.COLUMN_FILENAME,
			(long)(record.getDesktopMode() ? 16 : 0) + (long)(record.getNightMode() ? 32 : 0));
	...
	public void addBookmark(Record record) {
		...
		values.put(RecordUnit.COLUMN_TIME,
			record.getIconColor() + (long)(record.getDesktopMode() ? 16 : 0) + (long)(record.getNightMode() ? 32 : 0));
	...
	private Record getClipboard(Activity activity) {
		SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(activity);
		if (!sp.getBoolean("sp_clipboard_record", false))
			return null;
		String clipboardEntry = ClipboardUnit.getPrimary(activity);
		if (clipboardEntry == null || !URLUtil.isValidUrl(clipboardEntry))
			return null;
		return new Record(

...
public class NinjaWebViewClient extends WebViewClient {
	...
	public void onLoadResource(WebView view, String url) {
		...
		if (ninjaWebView.isFingerPrintProtection())
			view.evaluateJavascript(
				"var test=document.querySelector(\"a[ping]\"); if(test!==null){test.removeAttribute('ping')};", null);
		...
		if (view.getSettings().getUseWideViewPort() && (view.getWidth() < 1300))
			view.evaluateJavascript(
				"document.querySelector('meta[name=\"viewport\"]').setAttribute('content', 'width=1200px');", null);
